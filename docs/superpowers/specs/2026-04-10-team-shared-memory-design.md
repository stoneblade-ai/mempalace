...
Add team-shared memory to Cortex via a two-layer architecture: **private layer** (local, unchanged) + **team layer** (remote server). Individual data stays on the user's machine; shared knowledge lives on a team server. AI tools see a unified view through the existing MCP interface.
...
| Operation | Route |
|---|---|
| `cortex_search` | Parallel query local + remote, dedupe, merge via RRF |
| `cortex_status` | Fetch both, label sources |
| `cortex_list_wings` | Merge both, label `[local]` / `[team]` |
| `cortex_list_rooms` | Route to layer based on wing source |
| `cortex_add_drawer` | Always writes to local. Use `cortex_publish` to push to team |
| `cortex_delete_drawer` | Route by drawer ID prefix (`local_` / `team_`, no prefix = local). If drawer has `published_as`, warn user that team copy still exists |
| `cortex_publish` | **New tool.** Read local drawer -> POST to remote |
| `cortex_kg_*` | Route by `layer` parameter, default local |

### Publish Tool

```
cortex_publish(drawer_id, target_wing=None, target_room=None)
```
...
```bash
cortex search "auth decision"                  # both layers (default)
cortex search "auth decision" --layer local     # local only
cortex search "auth decision" --layer team      # team only

cortex mine ~/chats/ --mode convos              # writes local (default)
cortex mine ~/meeting-notes/ --publish --wing shared     # mines to local, then publishes to team
```
...
```bash
# Publish
cortex publish <drawer_id>
cortex publish <drawer_id> --wing new_wing --room new_room
cortex publish --wing auth-migration --room decisions   # batch by filter (see below)

# Team configuration
cortex team init          # Interactive setup: server URL + API key
cortex team status        # Connection status + permissions
cortex team whoami        # Current user + accessible wings

# Server management (admin)
cortex team serve                              # Start team server
cortex team serve --port 8900 --host 0.0.0.0
cortex team add-user --id kai --role member --read-wings frontend,shared,backend --write-wings frontend,shared
# → generates and prints API key: ak_a3f8b2c1... (shown once, never stored in plaintext)
cortex team remove-user --id kai
cortex team rotate-key --id kai   # generates new key, old key valid for 24h grace period
```
...
```bash
cortex publish --kg --subject "Maya" --predicate "completed" --object "auth-migration"
```
...
- API key can also be provided via `CORTEX_TEAM_API_KEY` environment variable (takes precedence over config.json). Recommended for CI/shared machines to avoid key in config file.
...
- API Key auth only (OAuth/SSO deferred). Supports env var `CORTEX_TEAM_API_KEY` as alternative to config file.
