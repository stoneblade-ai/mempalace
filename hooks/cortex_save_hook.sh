#!/bin/bash
# CORTEX SAVE HOOK — Auto-save every N exchanges
#
...
#       "hooks": [{
#         "type": "command",
#         "command": "/absolute/path/to/cortex_save_hook.sh",
#         "timeout": 30
#       }]
#     }]
#   }
...
#
# === CORTEX CLI ===
# This repo uses: cortex mine <dir>
# or:            cortex mine <dir> --mode convos
# Set CORTEX_DIR below if you want the hook to auto-ingest after blocking.
# Leave blank to rely on the AI's own save instructions.
#
# === CONFIGURATION ===

SAVE_INTERVAL=15  # Save every N human messages (adjust to taste)
STATE_DIR="$HOME/.cortex/hook_state"
mkdir -p "$STATE_DIR"

# Optional: set to the directory you want auto-ingested on each save trigger.
# Example: CORTEX_DIR="$HOME/conversations"
...
# Log for debugging (check ~/.cortex/hook_state/hook.log)
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    # Optional: run cortex ingest in background if CORTEX_DIR is set
    if [ -n "$CORTEX_DIR" ] && [ -d "$CORTEX_DIR" ]; then
        SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        REPO_DIR="$(dirname "$SCRIPT_DIR")"
        python3 -m cortex mine "$CORTEX_DIR" >> "$STATE_DIR/hook.log" 2>&1 &
    fi
