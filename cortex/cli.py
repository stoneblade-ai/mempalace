#!/usr/bin/env python3
"""
Cortex — Give your AI a memory. No API key required.
...
Commands:
    ...
    cortex split <dir>                 Split concatenated mega-files into per-session files
    cortex mine <dir>                  Mine project files (default)
    cortex mine <dir> --mode convos    Mine conversation exports
    cortex search "query"              Find anything, exact words
    cortex mcp                         Show MCP setup command
    cortex wake-up                     Show L0 + L1 wake-up context
    cortex wake-up --wing my_app       Wake-up for a specific project
    cortex status                      Show what's been filed

Examples:
    cortex init ~/projects/my_app
    cortex mine ~/projects/my_app
    cortex mine ~/chats/claude-sessions --mode convos
    cortex search "why did we switch to GraphQL"
    cortex search "pricing discussion" --wing my_app --room costs
"""
...
def cmd_repair(args):
    ...
    new_col = client.create_collection("cortex_drawers")
...
def cmd_mcp(args):
    ...
    print("Cortex MCP quick setup:")
    print(f"  claude mcp add cortex -- {server_cmd}")
...
def cmd_compress(args):
    ...
    if not args.dry_run:
        try:
            comp_col = client.get_or_create_collection("cortex_compressed")
            ...
            print(
                f"  Stored {len(compressed_entries)} compressed drawers in 'cortex_compressed' collection."
            )
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Cortex — Give your AI a memory. No API key required.",
    ...
    p_mine.add_argument(
        "--agent",
        default="cortex",
        help="Your name — recorded on every drawer (default: cortex)",
    )
    ...
    sub.add_parser(
        "mcp",
        help="Show MCP setup command for connecting Cortex to your AI client",
    )
