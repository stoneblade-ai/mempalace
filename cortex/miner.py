...
def load_config(project_dir: str) -> dict:
    """Load cortex.yaml from project directory (falls back to mempal.yaml)."""
    import yaml

    config_path = Path(project_dir).expanduser().resolve() / "cortex.yaml"
    if not config_path.exists():
        ...
        if legacy_path.exists():
            ...
            print(f"ERROR: No cortex.yaml found in {project_dir}")
            print(f"Run: cortex init {project_dir}")
            sys.exit(1)
...
def mine(
    ...
    print('\n  Next: cortex search "what you\'re looking for"')
    print(f"{'=' * 55}\n")
...
def status(cortex_path: str):
    ...
    print(f"\n{'=' * 55}")
    print(f"  Cortex Status — {len(metas)} drawers")
    print(f"{'=' * 55}\n")
