#!/usr/bin/env python3
"""
onboarding.py — Cortex first-run setup.

Asks the user:
  1. How they're using Cortex (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Cortex knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m cortex.onboarding
    or: cortex init
"""

from pathlib import Path
from cortex.entity_registry import EntityRegistry
from cortex.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Cortex")
    print("""
  Cortex is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Cortex?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Cortex distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from cortex.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    cortex_dir = Path(config_dir) if config_dir else Path.home() / ".cortex"
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by cortex init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (cortex_dir / "critical_facts.md").write_text("\n".join(facts_lines), encoding="utf-8")
...
def run_onboarding(
  ...
  Cortex will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.cortex/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.cortex/critical_facts.md")
