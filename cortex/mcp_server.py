...
Tools (read):
  ...
  cortex_list_wings      — all wings with drawer counts
  cortex_list_rooms      — rooms within a wing
  cortex_get_taxonomy    — full wing → room → count tree
  cortex_search          — semantic search, optional wing/room filter
  cortex_check_duplicate — check if content already exists before filing
...
AAAK_SPEC = """AAAK is a compressed memory dialect that Cortex uses for efficient storage.
...
TOOLS = {
    ...
    "cortex_get_aaak_spec": {
        "description": "Get the AAAK dialect specification — the compressed memory format Cortex uses. Call this if you need to read or write AAAK-compressed memories.",
        "input_schema": {"type": "object", "properties": {}},
