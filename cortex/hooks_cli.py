"""
Hook logic for Cortex — Python implementation of session-start, stop, and precompact hooks.
...
SAVE_INTERVAL = 15
STATE_DIR = Path.home() / ".cortex" / "hook_state"
...
def _maybe_auto_ingest():
    """If CORTEX_DIR is set and exists, run cortex mine in background."""
    cortex_dir = os.environ.get("CORTEX_DIR", "")
    if cortex_dir and os.path.isdir(cortex_dir):
        try:
            log_path = STATE_DIR / "hook.log"
            with open(log_path, "a") as log_f:
                subprocess.Popen(
                    [sys.executable, "-m", "cortex", "mine", cortex_dir],
                    stdout=log_f,
                    stderr=log_f,
                )
        except OSError:
...
def hook_stop(data: dict, harness: str):
    ...
    if since_last >= SAVE_INTERVAL and exchange_count > 0:
        ...
        # Optional: auto-ingest if CORTEX_DIR is set
        _maybe_auto_ingest()
