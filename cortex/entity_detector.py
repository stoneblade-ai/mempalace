...
Used by cortex init before mining begins.
...
PROJECT_VERB_PATTERNS = [
    ...
    r"\b{name}\s+v\d+\b",  # Cortex v2
    r"\b{name}\.py\b",  # cortex.py
    r"\b{name}-core\b",  # cortex-core (hyphen only, not underscore)
    r"\b{name}-local\b",
    r"\bimport\s+{name}\b",
...
def extract_candidates(text: str) -> dict:
    ...
    # Also find multi-word proper nouns (e.g. "Memory Cortex", "Claude Code")
