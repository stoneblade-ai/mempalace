...
Guide the user through a complete Cortex setup. Follow each step in order,
...
## Step 2: Check if cortex is already installed

Run `pip show cortex` to see if the package is already present. If it is,
...
If `pip install cortex` fails, try these fallbacks in order:

1. Try `pip3 install cortex`
2. Try `python -m pip install cortex` (or `python3 -m pip install cortex`)
...
Ask the user which project directory they want to initialize with Cortex.
Offer the current working directory as the default. Wait for their response
before continuing.

## Step 5: Initialize the Cortex

Run `cortex init <dir>` where `<dir>` is the directory from Step 4.

If this fails, report the error and stop.

## Step 6: Configure MCP server

Run the following command to register the Cortex MCP server with Claude:
...
Run `cortex status` and confirm the output shows a healthy Cortex.
...
- Use /cortex:mine to start adding data to their Cortex
- Use /cortex:search to query their Cortex and retrieve stored knowledge
